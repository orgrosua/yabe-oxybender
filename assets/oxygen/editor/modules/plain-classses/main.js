...
`).querySelector('#oxybender-plc-input');
...
`).querySelector('.oxybender-plc-action-container');
...
`).querySelector('#oxybender-plc-class-sort');
...
wp.hooks.addAction('oxybender-autocomplete-items-refresh', 'oxybender', () => {
    // wp hook filters. {value, color?, fontWeight?, namespace?}[]
    autocompleteItems = wp.hooks.applyFilters('oxybender-autocomplete-items', [], textInput.value);
});

wp.hooks.doAction('oxybender-autocomplete-items-refresh');

const tribute = new Tribute({
    containerClass: 'oxybender-tribute-container',
    ...
    values: async function (text, cb) {
        const filters = await wp.hooks.applyFilters('oxybender-autocomplete-items-query', autocompleteItems, text);
        cb(filters);
    },
...
watch([activeElementId, visibleElementPanel], (newVal, oldVal) => {
    ...
    if (newVal[0] && newVal[1]) {
        nextTick(() => {
            ...
            if (panelElementClassesEl.querySelector('.oxybender-plc-input') === null) {
                panelElementClassesEl.appendChild(containerAction);

                window.tippy('.oxybender-plc-class-sort', {
...
textInput.addEventListener('tribute-active-true', function (e) {
    if (menuAutocompleteItemeEl === null) {
        menuAutocompleteItemeEl = document.querySelector('.oxybender-tribute-container>ul');
    }
    nextTick(() => {
...
function previewAddClass(className) {
    postMessageToIframe({
        action: 'oxybender-preview-class',
        do: 'add',
